...
def main():
    ...
    parser.add_argument('--start', type=int, default=2872, help='Индекс начала тестовых данных')     #3258 2840
    ...
    # 1. Prepare Data
    X_train_s, X_test_s, y_train_s, y_test_s, Xw_train_s, Xw_test_s, yw_train_s, yw_test_s, y_train, y_test, scaler_y, scaler_yw, datas = prepare_data(data_path, test_start_index=test_start_index)
    ...
    checkpoint_filepath = checkpoint_dir+'Npred_LSTM.keras'
    ...
    checkpoint_filepath = checkpoint_dir+'Npred_sMLP.keras'
    model_checkpoint_callback = ModelCheckpoint(filepath=checkpoint_filepath, save_weights_only=False,
                                                monitor='val_loss', mode='min', save_best_only=True, verbose=1)
    if os.path.exists(checkpoint_filepath):
        print("Loading model for further training...")
    ...
    checkpoint_filepath = checkpoint_dir+'Npred_MLP.keras'
    model_checkpoint_callback = ModelCheckpoint(filepath=checkpoint_filepath, save_weights_only=False,
                                                monitor='val_loss', mode='min', save_best_only=True, verbose=1)
    if os.path.exists(checkpoint_filepath):
        print("Loading model for further training...")
    ...
    results['Boosting_with_window'] = scaler_yw.inverse_transform(yw_test_pred)
    ...
    checkpoint_filepath = checkpoint_dir+'Npred_LSTM_with_RW.keras'
    model_checkpoint_callback = ModelCheckpoint(filepath=checkpoint_filepath, save_weights_only=False,
                                                monitor='val_loss', mode='min', save_best_only=True, verbose=1)
    ...
    results['LSTM_with_Window'] = scaler_yw.inverse_transform(pred_lstm_s)
